...
namespace OpenFlightVRC.Extensions
{
    ...
    void OnDrawGizmos()
    {
        ...
        {
            UnityEngine.Gizmos.color = GetGizmoColorWithAlpha();
            drawColliderGizmo();
        }
    }
    ...
    {
        UnityEngine.Gizmos.color = GetGizmoColorWithAlpha() + new Color(0f, 0f, 0f, selectionGizmoAlphaAdjust);
        drawColliderGizmo();
    }
    ...
    protected virtual void drawColliderGizmo(){
        ...
        UnityEngine.Gizmos.matrix = Matrix4x4.TRS(transform.position, rotation, Vector3.one);
    ...
    protected virtual void handleColliderType()
    {
        ...
        if (zoneCollider is BoxCollider)
        {
            ...
            UnityEngine.Gizmos.DrawCube(boxCollider.center, Vector3.Scale(boxCollider.size, transform.lossyScale));
        }
        else if (zoneCollider is SphereCollider)
        {
            ...
            UnityEngine.Gizmos.DrawSphere(sphereCollider.center, sphereCollider.radius * radius);
        }
        ...
        else if (zoneCollider is MeshCollider)
        {
			...
			for (int i = 0; i < numPoints; i++)
			{
				...
				UnityEngine.Gizmos.DrawSphere(point, 0.1f);
			}
        }
    }
#endif
